...
class GPT2LatentSpace(torch.nn.Module):
    def __init__(self, config):
        ...
        if os.path.isfile(os.path.join(self.config.target[:-1], '.json')):
            with open(os.path.join(self.config.target[:-1], '.json')) as fp:
            ...
            with open(os.path.join(self.config.target[:-1], '.json'), 'w') as fp:
